...
describe(scriptName, () => {
  ...
  describe('when everyone votes negatively', () => {
    ...
    it('should let reported address retrieve compensation', async () => {
      ...
      //Test Task: expecting to be reverted because is not a contract
      await expect(
        env.lssGovernance.connect(adr.maliciousActor1).retrieveSmartContractCompensation(adr.maliciousActor1.address),
      ).to.be.revertedWith('LSS: Not Smart Contract Address');

      //Test Task: same test for the smart contract but setting an address adr.maliciuosActor2 as Smart Contract Address
      await expect(
        env.lssGovernance.connect(adr.maliciousActor2).retrieveSmartContractCompensation(adr.maliciousActor2.address),
      ).to.emit(env.lssGovernance, 'CompensationRetrieval').withArgs(
    ...
    it('should revert if other than the afflicted tries to retrieve', async () => {
      ...
      //Test Task: 
      await expect(
        env.lssGovernance.connect(adr.regularUser1).retrieveSmartContractCompensation(adr.regularUser1.address),
      ).to.be.revertedWith('LSS: No retribution assigned');
