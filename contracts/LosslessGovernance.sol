...
contract LosslessGovernance is
    ILssGovernance,
    Initializable,
    AccessControlUpgradeable,
    PausableUpgradeable
{
    uint256 public constant override LSS_TEAM_INDEX = 0;
    uint256 public constant override TOKEN_OWNER_INDEX = 1;
    uint256 public constant override COMMITEE_INDEX = 2;

    bytes32 public constant COMMITTEE_ROLE = keccak256("COMMITTEE_ROLE");

    uint256 public override committeeMembersCount;

    uint256 public override walletDisputePeriod;

    uint256 public compensationPercentage;

    uint256 public constant HUNDRED = 1e2;

    ILssReporting public override losslessReporting;
    ILssController public override losslessController;
    ILssStaking public override losslessStaking;
    ...
    function initialize(
        ILssReporting _losslessReporting,
        ILssController _losslessController,
        ILssStaking _losslessStaking,
        uint256 _walletDisputePeriod
    ) public initializer {
    ...
    modifier onlyLosslessPauseAdmin() {
        require(
            msg.sender == losslessController.pauseAdmin(),
            "LSS: Must be pauseAdmin"
        );
        _;
    }
    ...
    function isCommitteeMember(address _account)
        public
        view
        override
        returns (bool)
    ...
    function isReportSolved(uint256 _reportId)
        public
        view
        override
        returns (bool)
    ...
    function reportResolution(uint256 _reportId)
        public
        view
        override
        returns (bool)
    ...
    function setDisputePeriod(uint256 _timeFrame)
        public
        override
        onlyLosslessAdmin
        whenNotPaused
    {
        require(
            _timeFrame != walletDisputePeriod,
            "LSS: Already set to that amount"
        );
    ...
    function setCompensationAmount(uint256 _amount)
        public
        override
        onlyLosslessAdmin
    {
        ...
        require(
            _amount != compensationPercentage,
            "LSS: Already set to that amount"
        );
    ...
    function _getCommitteeMajorityReachedResult(uint256 _reportId)
        private
        view
        returns (bool isMajorityReached, bool result)
    {
    ...
    function addCommitteeMembers(address[] memory _members)
        public
        override
        onlyLosslessAdmin
        whenNotPaused
    ...
    /// @param _members Array of members to be added
    function removeCommitteeMembers(address[] memory _members)
        public
        override
        onlyLosslessAdmin
        whenNotPaused
    {
        require(
            committeeMembersCount >= _members.length,
            "LSS: Not enough members to remove"
        );
        ...
        for (uint256 i = 0; i < _members.length; ) {
            address newMember = _members[i];
            require(
                isCommitteeMember(newMember),
                "LSS: An address is not member"
            );
    ...
    function losslessVote(uint256 _reportId, bool _vote)
        public
        override
        onlyLosslessAdmin
        whenNotPaused
    ...
    /// @param _reportId Report to cast the vote
    /// @param _vote Resolution
    function tokenOwnersVote(uint256 _reportId, bool _vote)
        public
        override
        whenNotPaused
    {
        ...
        (, , , , ILERC20 reportTokens, , ) = losslessReporting.getReportInfo(
            _reportId
        );
        ...
        require(
            !reportVote.voted[TOKEN_OWNER_INDEX],
            "LSS: owners already voted"
        );
    ...
    /// @param _reportId Report to cast the vote
    /// @param _vote Resolution
    function committeeMemberVote(uint256 _reportId, bool _vote)
        public
        override
        whenNotPaused
    {
        ...
        require(
            ...
            "LSS: Must be a committee member"
        );
        require(isReportActive(_reportId), "LSS: report is not valid");
        ...
        require(
            !reportVote.committeeMemberVoted[msg.sender],
            "LSS: Member already voted"
        );
        ...
        (
            ...
            bool result
        ) = _getCommitteeMajorityReachedResult(_reportId);
    ...
    function resolveReport(uint256 _reportId) public override whenNotPaused {
        require(!isReportSolved(_reportId), "LSS: Report already solved");

        (, , , uint256 reportTimestamps, , , ) = losslessReporting
            .getReportInfo(_reportId);

        if (
            reportTimestamps + losslessReporting.reportLifetime() >
            block.timestamp
        ) {
    ...
    function _resolveActive(uint256 _reportId) private {
        ...
        ) = losslessReporting.getReportInfo(_reportId);
        ...
        if (getIsVoted(_reportId, LSS_TEAM_INDEX)) {
            voteCount += 1;
            if (getVote(_reportId, LSS_TEAM_INDEX)) {
                agreeCount += 1;
            }
        }
        if (getIsVoted(_reportId, TOKEN_OWNER_INDEX)) {
            voteCount += 1;
            if (getVote(_reportId, TOKEN_OWNER_INDEX)) {
                agreeCount += 1;
            }
        }

        (
            bool committeeResoluted,
            bool committeeResolution
        ) = _getCommitteeMajorityReachedResult(_reportId);
        if (committeeResoluted) {
            voteCount += 1;
        ...
        require(
            !(voteCount == 2 && agreeCount == 1),
            "LSS: Need another vote to untie"
        );
        ...
        if (agreeCount > (voteCount - agreeCount)) {
            ...
            for (uint256 i; i < reportedAddresses.length; ) {
                reportVote.amountReported += token.balanceOf(
                    reportedAddresses[i]
                );
                unchecked {
                    i++;
            ...
            proposedWalletOnReport[_reportId]
                .retrievalAmount = losslessController.retrieveBlacklistedFunds(
                reportedAddresses,
                token,
                _reportId
            );
    ...
    function _compensateAddresses(address[] memory _addresses) private {
        ...
        uint256 compensationAmount = (reportingAmount *
            compensationPercentage) / HUNDRED;

        for (uint256 i = 0; i < _addresses.length; ) {
            address singleAddress = _addresses[i];
            Compensation storage addressCompensation = compensation[
                singleAddress
            ];
    ...
    function proposeWallet(uint256 _reportId, address _wallet)
        public
        override
        whenNotPaused
    {
        ...
        ProposedWallet storage proposedWallet = proposedWalletOnReport[
            _reportId
        ];
    ...
    /// @dev Only can be run by the three pilars.
    /// @param _reportId Report to propose the wallet
    function rejectWallet(uint256 _reportId) public override whenNotPaused {
        (
            ,
            ,
            ,
        ...
        require(
            block.timestamp <= (proposedWallet.timestamp + walletDisputePeriod),
            "LSS: Dispute period closed"
        );
        require(reportTimestamps != 0, "LSS: Report does not exist");
        ...
        if (hasRole(COMMITTEE_ROLE, msg.sender)) {
            require(
                !proposedWallet
                    .memberVotesOnProposal[proposedWallet.proposal]
                    .memberVoted[msg.sender],
                "LSS: Already Voted"
            );
    ...
    /// @param _reportId Report to propose the wallet
    function retrieveFunds(uint256 _reportId) public override whenNotPaused {
        (
            ,
            ,
            ,
        ...
        require(
            block.timestamp >= (proposedWallet.timestamp + walletDisputePeriod),
            "LSS: Dispute period not closed"
        );
        require(reportTimestamps != 0, "LSS: Report does not exist");
        require(!proposedWallet.status, "LSS: Funds already claimed");
        require(proposedWallet.walletAccepted, "LSS: Wallet rejected");
        require(
            proposedWallet.wallet == msg.sender,
            "LSS: Only proposed adr can claim"
        );

        proposedWallet.status = true;

        require(
            reportTokens.transfer(msg.sender, proposedWallet.retrievalAmount),
    ...
    /// @param _reportId Report to propose the wallet
    function _determineProposedWallet(uint256 _reportId)
        private
        returns (bool)
    {
        ProposedWallet storage proposedWallet = proposedWalletOnReport[
        ...
        if (
            proposedWallet.committeeDisagree < (committeeMembersCount >> 2) + 1
        ) {
            agreementCount += 1;
        }
    ...
    /// Test Task Added by Eze Cerino
    /// @notice This lets an erroneously reported smart contract address to retrieve compensation
    function retrieveSmartContractCompensation(address _smrtCtrctAddress)
        public
        override
        whenNotPaused
    {
        // Check address correspond to a smart contract
        require(
            isContract(_smrtCtrctAddress),
            "LSS: Not Smart Contract Address"
        );
        // declare this address as payable
        address payable _smrtCtrctAddress;
        //same process as before
        require(
            !compensation[_smrtCtrctAddress].payed,
            "LSS: Already retrieved"
        );
        require(
            compensation[_smrtCtrctAddress].amount != 0,
            "LSS: No retribution assigned"
        );

        compensation[_smrtCtrctAddress].payed = true;

        losslessReporting.retrieveCompensation(
            _smrtCtrctAddress,
            compensation[_smrtCtrctAddress].amount
        );

        emit CompensationRetrieval(
            _smrtCtrctAddress,
            compensation[_smrtCtrctAddress].amount
        );
    ...
    function claimCommitteeReward(uint256 _reportId)
        public
        override
        whenNotPaused
    {
        ...
        require(
            reportVote.committeeMemberVoted[msg.sender],
            "LSS: Did not vote on report"
        );
        require(
            !reportVote.committeeMemberClaimed[msg.sender],
            "LSS: Already claimed"
        );
        ...
        uint256 compensationPerMember = ((reportVote.amountReported *
            committeeReward) / HUNDRED) / numberOfMembersVote;

        reportVote.committeeMemberClaimed[msg.sender] = true;

        require(
            reportTokens.transfer(msg.sender, compensationPerMember),
    ...
    function losslessClaim(uint256 _reportId)
        public
        override
        whenNotPaused
        onlyLosslessAdmin
    {
        ...
        uint256 amountToClaim = (reportVote.amountReported *
            losslessReporting.losslessReward()) / HUNDRED;
        reportVote.losslessPayed = true;
        require(
            reportTokens.transfer(losslessController.admin(), amountToClaim),
            "LSS: Reward transfer failed"
        );
